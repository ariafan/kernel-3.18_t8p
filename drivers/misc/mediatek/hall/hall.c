...
#define GPIO_HALL_EINT_PIN GPIO124	//move to dct
...
struct pinctrl_state *irqpins_default;
struct pinctrl_state *irqpins_cfg;
...
static int hall_setup_eint(void)
{
    ...
    if(irq_node)
    {
		...
		pinctrl_select_state(hallctrl, irqpins_cfg);
...
static int hall_probe(struct platform_device *dev)
{
	...
	irqpins_cfg = pinctrl_lookup_state(hallctrl, "hall_intpin_cfg");
	if (IS_ERR(irqpins_cfg)) {
		ret = PTR_ERR(irqpins_cfg);
		pr_err("Cannot find alsps hallctrl pin_cfg!\n");
	}

    //pinctrl_select_state(hallctrl, irqpins_cfg);
    /*
	curr_state = gpio_get_value(GPIO_HALL_EINT_PIN);
	printk("%s line %d curr_state %d\n",__func__,__LINE__, curr_state);
    */
